...
function containsTodayDate(text) {
  const normalized = normalizeText(text);

  return getDateVariants().some(date => {
...
/* =========================
   FIND TODAY'S QUIZ AREA
========================= */

function findTodayQuizStart(lines) {

  const variants = getDateVariants();

  console.log(
    "Looking for today's Pakistan date:"
  );
  ...
  /*
   * IMPORTANT:
   * We do NOT simply accept today's date anywhere
   * on the website.
   *
   * We find today's date and then look for
   * Question 1 near that date.
   */
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!containsTodayDate(current)) {
      continue;
    }

    console.log(
      `✓ Today's date found near line ${i}`
    );

    /*
     * Look forward for Question 1.
     * This prevents an old Question 1 appearing
     * before today's date from being selected.
     */

    for (
      let j = i;
      j < Math.min(i + 80, lines.length);
      j++
    ) {

      if (
        /^Question\s*(?:No\.?)?\s*[:\-]?\s*0?1$/i
          .test(lines[j])
      ) {

        console.log(
          `✓ Today's Question 1 found at line ${j}`
        );
  ...
  throw new Error(
    `TODAY'S QUIZ NOT FOUND: Today's Pakistan date (${getPakistanDate()}) was not found together with Question 1. Old quiz will NOT be published.`
  );
}
...
async function fetchQuiz() {
  ...
  for (
    ...
    /*
     * Do not allow Question numbers
     * to go backwards.
     */

    if (
      quiz.length > 0 &&
      number <= quiz[quiz.length - 1].number
    ) {
      continue;
    }
...
async function main() {
  ...
  console.log(
    "\nSTEP 1: FETCH + VERIFY TODAY'S QUIZ"
  );
